...
<div class="w-100 position-relative" style="background-image: url('{{$app->header_image_url}}'); height: 300px; margin-bottom: 60px;">

    <div class="position-absolute row w-100" style="bottom: -50px;">
        <img src="{{$app->icon_url}}" class="img-rounded bg-white ml-4" width="150" height="150">
        <h1 class="mt-auto ml-3 mb-0">{{$app->name}}</h1>
        <a href="{{ route('apps.topics.create', ['appId' => $app->id]) }}" class="btn btn-primary mt-3 mr-5 ml-auto">トピック作成</a>
    </div>
